...
export default function CustomerLayout() {
  ...
  // Lock scroll when mobile menu is open to prevent background thrashing
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex flex-col" dir={i18n.dir()}>
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        {/* Social Bar (Dynamic RTL/LTR via justify-end) */}
        {(storefrontConfig?.socialLinks?.instagram || storefrontConfig?.socialLinks?.facebook || storefrontConfig?.socialLinks?.tiktok || storefrontConfig?.socialLinks?.twitter || storefrontConfig?.socialLinks?.youtube) && (
          <div className="bg-gray-50 border-b border-gray-100 hidden md:block">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-end py-1.5">
              <div className="flex gap-4">
                ...
                {storefrontConfig?.socialLinks?.twitter && (
                  <a href={storefrontConfig.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-black transition-colors">
                    ...
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" /></svg>
                  </a>
                )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <button className="md:hidden p-2 -ml-2 rtl:-ml-0 rtl:-mr-2 text-gray-600 hover:text-brand-600" onClick={() => setIsMobileMenuOpen(true)}>
            ...
            <div className="flex-shrink-0 flex items-center">
              <Link to="/" className="text-xl md:text-2xl font-black tracking-tighter text-gray-900 flex items-center gap-2">
            ...
            {/* Main Navigation */}
            <nav className={`hidden md:flex items-center ${i18n.dir() === 'rtl' ? 'gap-12' : 'gap-8'}`}>
              ...
              <a href={storefrontConfig?.whatsappNumber ? `https://wa.me/${storefrontConfig.whatsappNumber}` : '#'} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors">
                {t('common.contactUs')}
              </a>
            </nav>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[100] md:hidden h-[100dvh] flex">
          {/* Backdrop */}
          <div className="absolute inset-0 bg-black/50 transition-opacity" onClick={() => setIsMobileMenuOpen(false)}></div>
          
          {/* Menu Drawer */}
          <div className={`relative w-[80%] max-w-sm bg-white shadow-2xl flex flex-col h-[100dvh] transform transition-transform ${i18n.dir() === 'rtl' ? 'ml-auto' : 'mr-auto'}`}>
             <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                 <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-black tracking-tighter text-gray-900 flex items-center gap-2">
                   <div className="w-8 h-8 bg-brand-900 rounded-lg flex items-center justify-center text-white text-xl font-serif">G</div>
                   <span>Gold<span className="text-brand-600">Bag</span></span>
                 </Link>
                 <button className="p-2 text-gray-400 hover:text-gray-900 rounded-full hover:bg-gray-200 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                   <X size={24} />
                 </button>
             </div>
             <div className="p-6 space-y-6 flex flex-col flex-1 overflow-y-auto">
                <nav className="flex flex-col space-y-4">
                  <Link to="/" className="text-lg font-semibold text-gray-900 hover:text-brand-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('common.home')}</Link>
                  <Link to="/products" className="text-lg font-semibold text-gray-900 hover:text-brand-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('common.categories')}</Link>
                </nav>
                ...
                {!isAuthenticated ? (
                  <div className="flex flex-col gap-3">
                    <Link to="/login" className="w-full text-center py-2.5 text-brand-600 font-semibold border border-brand-200 rounded-lg bg-brand-50 hover:bg-brand-100 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('common.signIn')}</Link>
                    <Link to="/register" className="w-full text-center py-2.5 bg-brand-600 text-white font-semibold rounded-lg hover:bg-brand-700 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>{t('common.signUp')}</Link>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
                    <Link to="/profile" className="text-base font-medium text-gray-700 hover:text-brand-600" onClick={() => setIsMobileMenuOpen(false)}>{t('common.myProfile')}</Link>
                    <Link to="/my-orders" className="text-base font-medium text-gray-700 hover:text-brand-600" onClick={() => setIsMobileMenuOpen(false)}>{t('common.myOrders')}</Link>
                    <button onClick={() => { handleSignOut(); setIsMobileMenuOpen(false); }} className="text-left rtl:text-right text-base font-medium text-red-600 hover:text-red-700 mt-2">{t('common.signOut')}</button>
                  </div>
                )}

                <div className="mt-auto pt-6 border-t border-gray-100">
                  <a href={storefrontConfig?.whatsappNumber ? `https://wa.me/${storefrontConfig.whatsappNumber}` : '#'} target="_blank" rel="noopener noreferrer" className="text-base font-semibold text-gray-600 hover:text-brand-600 transition-colors flex items-center gap-2">
                    {t('common.contactUs')}
                  </a>
                </div>
             </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div id="contact">
              ...
              <ul className="space-y-2">
                <li>
                  <a href={storefrontConfig?.whatsappNumber ? `https://wa.me/${storefrontConfig.whatsappNumber}` : '#'} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-500 hover:text-brand-600 cursor-pointer">
                    {t('common.contactUs')}
                  </a>
                </li>
              </ul>
